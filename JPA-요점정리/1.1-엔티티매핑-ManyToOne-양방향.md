...
먼저 Employee 객체는 아래와 같이 세팅해주고 있다. 이번에는 Department 객체를 직접 세팅해준다.
...
Department 객체는 아래와 같이 세팅해주고 있다. 이번에는 Department 객체 내의 list 타입인 employees 내에 객체를 추가해주는 부분을 주석처리했다.<br>
...
목요일에 다시 설명 정리, 예제도 추가. 회사일이 바빠서 여기까지만 정리해야게씀<br>

<br>

## 테스트 코드 4) 연관관계 편의메서드 사용해보기

Employee 라는 도메인 객체 내에서 필요한 객체에 대한 로직을 담당하도록 하는 방식이다. 예를 들면 Employee.java 내에 `assignDept(Department)` 라는 메서드를 아래와 같이 추가하는 방식이다.
