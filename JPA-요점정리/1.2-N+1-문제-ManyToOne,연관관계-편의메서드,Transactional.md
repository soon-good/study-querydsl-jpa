...
위에서는 엔티티 클래스 Employee에 연관관계 편의 메서드들을 정의했다. 연관관계의 주인 역할을 하는 Employee 객체 입장에서 맞은편 연관관계인 Department 내에 말려 들어가는 컬렉션 객체인 `List <Employee> employees` 에 대해 삽입/삭제/수정을 하는 연산들을 직접 작성했다. 이렇게 하는 이유는 트랜잭션이 끝나고 나서 JPA 는 flush() 를 수행하는데 이때 테이블에는 데이터가 반영되더라도 객체에는 반영되지 않기에 반대편에 대한 연산도 추가해주는 것이다.<br>
...
- Employee targetEmployee = employeeRepository.findById(id).orElseThrow (() -> new NoEmployeeFoundException("해당 사원이 존재하지 않습니다."))
...
오늘공부는 여기까지. 여기까지 정리하는데 20분이 걸렸고 다시 읽으면서 오타 찾는데 5분 + 산만하게 딴짓 5분 이렇게 30분 걸렸다. 오늘 공부는 여기까지고, 아마도 이번 주 내로 정리는 마무리 될 것 같다. 현재 회사에서 진행하고 있는 프로젝트에는 2차 캐시, 엔티티 캐시를 미리 미리 정리해두는 것이 더 시급하기는 하지만, 이번 문서를 정리하는 것 역시도 오랫동안 미뤄온 작업이기에 이번주에 시작하게 된게 나쁜 선택은 아닌 것 같다.
